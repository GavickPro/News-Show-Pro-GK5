...
class NSP_GK5_xml_file_View extends NSP_GK5_View {
	// header generator
	static function header($config, $item) {
		if($config['news_content_header_pos'] != 'disabled') {
			...
			if(NSP_GK5_xml_file_View::image($config, $item, true, true) != '') {
				$class .= ' has-image';
			}
	...
	static function text($config, $item, $readmore) {
		if($config['news_content_text_pos'] != 'disabled') {
			...
			$item['text'] = NSP_GK5_xml_file_View::textPlugins($item['text'], $config);
			$link = $item['url'];
			//
